    ...
    size_t* filesize_out = nullptr) 
{
    ...
    char* file_contents = (char*)ArenaAlloc(arena, filesize);
    file.seekg(0);
...
const char** get_required_instance_extensions(Arena* arena, u32& num_required_extensions)
{
    ...
    ME_ASSERT(glfw_extensions != nullptr && "Vulkan not available on this machine!");
    u32 num_extra_extensions = validation_layers_enabled ? 1 : 0;
    const char** extension_names = (const char**)ArenaAlloc(arena, sizeof(char*) * (glfw_extension_count + num_extra_extensions));
...
bool checkValidationLayerSupport(Arena* arena)
{
    ...
    VkLayerProperties* available_layers = (VkLayerProperties*)ArenaAlloc(arena, sizeof(VkLayerProperties) * layer_count);
...
VkInstance createInstance(Arena* arena)
{
    ...
    ArenaTemp extensions_arena = ArenaTempInit(arena);
    u32 supported_extension_count = 0;
    vkEnumerateInstanceExtensionProperties(nullptr, &supported_extension_count, nullptr);
    VkExtensionProperties* supported_extensions = (VkExtensionProperties*)ArenaAlloc(&extensions_arena, sizeof(VkExtensionProperties) * supported_extension_count);
    vkEnumerateInstanceExtensionProperties(nullptr, &supported_extension_count, supported_extensions);

    ME_ASSERT(num_required_extensions <= supported_extension_count);
    ...
    VkSurfaceKHR surface)
{
    ...
    ArenaTemp arena_temp = ArenaTempInit(arena); // only need the family properties temporarily
    VkQueueFamilyProperties* queue_family_properties = (VkQueueFamilyProperties*)ArenaAlloc(&arena_temp, sizeof(VkQueueFamilyProperties) * queue_family_count);
    ...
    VkSurfaceKHR surface)
{
    ...
    swapchain_images_buffer.data = (VkImage*)ArenaAlloc(arena, sizeof(VkImage) * swapchain_images_buffer.size);
    ...
    VkPhysicalDevice physical_device)
{
    ...
    ArenaTemp arena_temp = ArenaTempInit(arena);
    VkExtensionProperties* available_extensions = (VkExtensionProperties*)ArenaAlloc(&arena_temp, sizeof(VkExtensionProperties) * device_extension_count);
    ...
    VkSurfaceKHR surface)
{
    ...
    VkPhysicalDevice* physical_devices = (VkPhysicalDevice*)ArenaAlloc(arena, sizeof(VkPhysicalDevice) * device_count);
    ...
    VkDeviceQueueCreateInfo*& q_create_infos_out)
{
    ...
    VkDeviceQueueCreateInfo* q_create_infos = (VkDeviceQueueCreateInfo*)ArenaAlloc(arena, sizeof(VkDeviceQueueCreateInfo) * num_unique_queue_families);
    ...
    VkSurfaceKHR surface)
{
    ...
    ME_ASSERT(queue_create_infos != nullptr);
    ...
    const SwapchainInfo& swapchain_info)
{
    ...
    ME_ASSERT(image_views.size > 0);
    image_views.data = ArenaAllocType(arena, VkImageView, image_views.size);
...
{
    VkFramebuffer* swapchain_framebuffers = ArenaAllocType(arena, VkFramebuffer, swapchain_image_views.size);
    ...
    VkCommandPool command_pool)
{
    ...
    command_buffers.data = ArenaAllocType(arena, VkCommandBuffer, MAX_FRAMES_IN_FLIGHT);
    ...
    u32 current_frame)
{
    ...
    ME_ASSERT(image_index < swapchain_framebuffers.size);
...
{
    img_available_semaphores.data = ArenaAllocType(arena, VkSemaphore, MAX_FRAMES_IN_FLIGHT);
    img_available_semaphores.size = sizeof(VkSemaphore) * MAX_FRAMES_IN_FLIGHT;
    render_finished_semaphores.data = ArenaAllocType(arena, VkSemaphore, MAX_FRAMES_IN_FLIGHT);
    render_finished_semaphores.size = sizeof(VkSemaphore) * MAX_FRAMES_IN_FLIGHT;
    inflight_fences.data = ArenaAllocType(arena, VkFence, MAX_FRAMES_IN_FLIGHT);
    ...
    const BufferView<VkBuffer>& uniform_buffers)
{
    ...
    BufferView<VkDescriptorSetLayout> desc_set_layouts = {ArenaAllocType(arena, VkDescriptorSetLayout, MAX_FRAMES_IN_FLIGHT), MAX_FRAMES_IN_FLIGHT};
    for(u32 i = 0; i < MAX_FRAMES_IN_FLIGHT; i++)
    { // populate desc_set_layouts with the specified layout (yes, these copies of the layout are necessary)
        ME_MEMCPY(&desc_set_layouts.data[i], &layout, sizeof(layout));
    }
...
{    
    ...
    void* program_mem = SYSTEM_MALLOC(program_max_mem);
    RuntimeData runtime;
    runtime.arena = ArenaInit(program_mem, program_max_mem, "MainArena");
    ...
    void* swapchain_arena_mem = ArenaAlloc(&arena, swapchain_arena_size);
    runtime.swapchain_arena = ArenaInit(swapchain_arena_mem, swapchain_arena_size, "SwapchainArena");;
...
void destroy_swapchain(RuntimeData& runtime)
{
    ...
    ArenaClear(&runtime.swapchain_arena);
}
